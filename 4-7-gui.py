# -*- coding: utf-8 -*-
"""
Created on Fri Apr  2 11:26:22 2021
...
class worker(QObject):
    ...
    def run(self):
        run_file(self.filepath+' '+self.params)
...
class Ui_RunWindow(object):
    def setupUi(self, RunWindow):
        ...
        self.toolButton.clicked.connect(lambda: self.startRunList(0))
        ...
        self.toolButton_4.clicked.connect(lambda: self.stopRunList(0))
        ...
        self.toolButton_2.clicked.connect(lambda: self.startRunList(1))
        ...
        self.toolButton_3.clicked.connect(lambda: self.startRunList(2))
        ...
        self.toolButton_11.clicked.connect(lambda: self.startRunList(3))
        ...
        self.pushButton_21.clicked.connect(self.newValueToolList)
...
#Update the tool list table when there's a new value
    def newValueToolList(self):
        
        row = self.nextAvailableRow()
        
        item = self.RunListTable_2.item(row, 0)
        
        row_value = self.updateToolList(row)
...
#Remove from Tool List      
    def removeToolList(self, button):
        ...
        batch=BatchStatement()
        batch.add(SimpleStatement("DELETE FROM Tool_Specification WHERE tool_name=%s"), ([tool.tool_name]))
        ...
        item = item = self.RunListTable_2.item(0, 0)
...
#Update Tool List      
    ...
    def deleteFromThreadList(self,row):
    ...
    def TLisEmpty(self):
        ...
        if empty == len(self.threadList):
            return True
        return False

    def stopRunList(self,row):
        if not self.TLisEmpty():
            for thread in self.threadList:
                if thread[0] == row and thread[1].isRunning():
                    #thread[1].pause()
                    #print(thread)
                    return

#Start button for Run list area
    def startRunList(self,row):
        if not self.TLisEmpty():
            for thread in self.threadList:
                if thread[0] == row and not thread[1].isRunning():
                    thread[1].start()
                    return


        nameOfRun = self.RunListTable.item(row, 0).text() #gets Name of Run
        ...
        try:
            ...
            self.worker = worker()
            self.worker.filepath = filepath
            self.worker.params = params

            self.worker.moveToThread(self.thread)
            #self.thread.started.connect(self.printThreadList) #to check if thread was added to list
            self.thread.started.connect(self.worker.run)
            self.worker.finished.connect(lambda: self.deleteFromThreadList(row))
            #self.worker.finished.connect(self.printThreadList) #to check if thread deleted from list
            self.worker.finished.connect(self.worker.deleteLater)
            self.threadList.append([row,self.thread])
            self.threadList[-1][1].start()
